...
const RatingBar = ({ ratingCategory, ratingValue }: RatingBarProps) => {
    return (
    <dd className="flex flex-col mb-2">
        ...
        <div className="bg-slate-200 rounded-md h-2.5">
            <div className="bg-yellow-400 h-2.5 rounded-md" style={{"width" : `${(ratingValue/5)*100}%`}}></div>
        </div>
    </dd>
)}
...
const VenueDetails: React.FC<VenueDetailsProps> = ({ venue, onClose }) => {
    return (
        <div className="absolute top-3 left-3 w-[300px] bg-white z-10 rounded-md shadow-md">
            <div className="h-44 bg-cover bg-center rounded-t-md relative" style={{ backgroundImage: `url('${venue.thumbnailUri}')` }}>
                <div className="absolute top-1 right-1 p-1 bg-white rounded-full cursor-pointer" onClick={onClose}>
            ...
            <div className="p-2">
                <h2 className="font-semibold text-xl/none text-blue-950">{venue.name}</h2>
                <div className="text-slate-400 text-[12px]">{venue.location.address}</div>
                <div className="text-slate-500 text-xs mt-3 mb-3">{venue.description}</div>
                ...
                <div className="flex justify-evenly mb-1">
                    {venue.contact.phone && <a href={`tel:${venue.contact.phone}`}> 
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 0 0 2.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 0 1-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 0 0-1.091-.852H4.5A2.25 2.25 0 0 0 2.25 4.5v2.25Z" />
                        </svg>
                    </a>}
                    {venue.contact.twitterUri && <a href={venue.contact.twitterUri}>
