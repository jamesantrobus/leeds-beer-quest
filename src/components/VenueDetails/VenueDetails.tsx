  ...
  return (
    <dd className="flex flex-col mb-2" data-cy={`rating-${ratingCategory}`}>
      <div className="text-[11px] font-medium text-slate-800">
        {ratingCategory} ({ratingValue}/5)
      </div>
      <div className="bg-slate-200 rounded-md h-2.5">
        <div
          className="bg-yellow-400 h-2.5 rounded-md"
          data-cy="bar"
...
const VenueDetails: React.FC<VenueDetailsProps> = ({ venue, onClose }) => {
  return (
    <div className="absolute top-3 left-3 w-[300px] bg-white z-10 rounded-md shadow-md" data-cy="venue-details">
      <div
        className="h-44 bg-cover bg-center rounded-t-md relative"
        style={{ backgroundImage: `url('${venue.thumbnailUri}')` }}
        data-cy="image"
      >
        <div
      ...
      <div className="p-2">
        <h2 className="font-semibold text-xl/none text-blue-950" data-cy="name">
          {venue.name}
        </h2>
        <div className="text-slate-400 text-[12px]" data-cy="address">
          {venue.location.address}
        </div>
        <div className="text-slate-500 text-xs mt-3 mb-3" data-cy="description">
          {venue.description}
        </div>
        ...
        <div className="flex justify-evenly mb-1">
          {venue.contact.phone && (
            <a href={`tel:${venue.contact.phone}`} data-cy="phone-link">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
          ...
          {venue.contact.twitterUri && (
            <a href={venue.contact.twitterUri} data-cy="twitter-link">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="currentColor"
